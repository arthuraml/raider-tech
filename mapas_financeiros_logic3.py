    ...
    def gerar_dbf_mapas_financeiros(self):
        ...
        try:
            ...
            table = dbf.Table(nome_arquivo_dbf,
                ...
                'REM_PIXCJ N(12, 2); REM_PIXQR N(12, 2); REM_DEBC N(12, 2); '
                'REM_CRED N(12, 2); SALVO N(1, 0); FECHADO N(1, 0); OBS_G C(255)',
                codepage='cp850')
    ...
    def gerar_dbf_vendas(self):
        ...
        try:
            table = dbf.Table(nome_arquivo_dbf,
                            ...
                            'CANCELAD N(1, 0); CANCELTIME N(20, 0); SUBTOTAL N(12, 2); PRECO_ATAC N(12, 2); '
                            'PRECO_VAR N(12, 2); PRECO_APL C(255); PONTA L; PROMO L; COD_VEND N(6, 0); '
                            'NOME_VEND C(255); LOJA C(255); TERMINAL N(6, 0); PAGAMENT L; '
                            'FORMA_PAGT C(255); PARCELAS N(3, 0); CHEQUE L; CLIENT_NOM C(255); '
                            'CLIENT_CPF C(255); EXTERNA N(1, 0); EXT_DIF N(12, 2); DATA_DBF C(10)',
                            codepage='cp850')
            table.open(mode=dbf.READ_WRITE)
            ...
            for ix in range(self.ui.tableWidget.rowCount()):
                ...
                if checkbox.isChecked():
                    ...
                    for abertura_timestamp, fechamento_timestamp in timestamps:
                        ...
                        externa, externa_dif
                        FROM vendas 
                        WHERE loja_cod = %s AND ((pagamento_timestamp >= %s AND pagamento_timestamp < %s) OR (cancelamento_timestamp >= %s AND cancelamento_timestamp < %s) OR (estorno_timestamp >= %s AND estorno_timestamp < %s))
                        """
                        cursor.execute(query_vendas, (loja_cod, abertura_timestamp, fechamento_timestamp, abertura_timestamp, fechamento_timestamp, abertura_timestamp, fechamento_timestamp))
